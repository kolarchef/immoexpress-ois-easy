...
export const ACTION_IDS = {
  ...
  RECHTSBERATUNG: "rechtsberatung", // <-- NEU für Brainy
} as const;

// --- Webhook Actions ---------------------------------------------------------

/** Send a PDF generation request (Quick-Check) */
export const sendPdfQuick = (params: PdfWebhookParams) => sendAction(ACTION_IDS.PDF_QUICK, params);

/** Send a PDF generation request (Classic / Exposé-Style) */
export const sendPdfClassic = (params: PdfWebhookParams) => sendAction(ACTION_IDS.PDF_CLASSIC, params);

/** Send a PDF generation request (Investment-Analyse) */
export const sendPdfInvestment = (params: PdfWebhookParams) => sendAction(ACTION_IDS.PDF_INVESTMENT, params);

/** Send a KI Video Rundgang request */
export const sendVideoKi = (params: VideoWebhookParams) => sendAction(ACTION_IDS.VIDEO_KI, params);
...
export const sendGetDistrictStats = (bezirk: string) => sendAction(ACTION_IDS.GET_DISTRICT_STATS, { bezirk });

/** KiezCheck: fetch school data */
export const sendGetSchoolData = (bezirk: string) => sendAction(ACTION_IDS.GET_SCHOOL_DATA, { bezirk });

/** KiezCheck: fetch live transit data */
export const sendGetTransitLive = (bezirk: string) => sendAction(ACTION_IDS.GET_TRANSIT_LIVE, { bezirk });

/** NEU: Rechtsberatung (Brainy) an Make senden */
export const sendRechtsberatung = (question: string, user_name: string, userIp: string) =>
